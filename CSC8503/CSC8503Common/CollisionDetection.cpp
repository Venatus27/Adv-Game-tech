...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume, RayCollision& collision) {
	...
	Quaternion orientation = worldTransform.GetOrientation();
	Vector3 position = worldTransform.GetPosition(); //A

	Matrix3 transform = Matrix3(orientation);
	Vector3 uv = transform * Vector3(0, 1, 0); //upvalue B, origin + world space up

	Vector3 top = position + (uv * (volume.GetHalfHeight() - volume.GetRadius()));
	Vector3 bottom = position - (uv * (volume.GetHalfHeight() - volume.GetRadius()));

	Vector3 localRayPos = r.GetPosition() - position;
	Vector3 OriginTop = top - position;

	Vector3 thirdPoint = Vector3::Cross(localRayPos.Normalised(), OriginTop.Normalised()); //AXB
	thirdPoint += position;

	Plane p = Plane::PlaneFromTri(position, OriginTop, thirdPoint);
	RayPlaneIntersection(r, p, collision);

	Vector3 VA = collision.collidedAt - top;
	Vector3 VB = top - position;

	Vector3 pen = collision.collidedAt - top;
	//Debug::DrawLine(r.GetPosition(), collision.collidedAt, Vector4(1, 1, 1, 1), 10);

	if (Vector3::Dot(VA, VB) < 0) { //top sphere
		if (pen.Length() < volume.GetRadius()) {
			collision.rayDistance = (r.GetPosition() - collision.collidedAt).Length();
			return true;
		}
	}

	VA = collision.collidedAt - bottom;
	VB = bottom - position;
	pen = collision.collidedAt - bottom;

	if (Vector3::Dot(VA, VB) < 0) { //bottom sphere
		if (pen.Length() < volume.GetRadius()) {
			collision.rayDistance = (r.GetPosition() - collision.collidedAt).Length();
			return true;
		}
	...
	Vector3 d = position + uv * Maths::Clamp((Vector3::Dot(collision.collidedAt - position, uv)), -volume.GetHalfHeight(), volume.GetHalfHeight());
	VA = (d - collision.collidedAt);

	if (VA.Length() < volume.GetRadius()) {
		collision.rayDistance = (r.GetPosition() - collision.collidedAt).Length();
		return true;
	}
...
bool CollisionDetection::OBBSphereIntersection(const OBBVolume& volumeA, const Transform& worldTransformA,
	...
	Vector3 OBBTransform = invTransform * worldTransformA.GetPosition();
	Vector3 SphereTransform = invTransform * worldTransformB.GetPosition();

	Vector3 boxSize = volumeA.GetHalfDimensions();

	Vector3 delta = SphereTransform - OBBTransform;
...
bool CollisionDetection::SphereCapsuleIntersection(
	...
	Quaternion orientation = worldTransformA.GetOrientation();
	Vector3 position = worldTransformA.GetPosition(); //A

	Matrix3 transform = Matrix3(orientation);
	Vector3 uv = transform * Vector3(0, 1, 0); //upvalue B, origin + world space up
											   
	Vector3 d = position + uv * Maths::Clamp((Vector3::Dot(worldTransformB.GetPosition() - position, uv)), -volumeA.GetHalfHeight() + volumeA.GetRadius(), volumeA.GetHalfHeight() - volumeA.GetRadius());
	Vector3 distance = (d - worldTransformB.GetPosition());

	float radii = volumeA.GetRadius() + volumeB.GetRadius();


	if (distance.Length() < radii) {
		SphereVolume tempSphere = SphereVolume(volumeA.GetRadius()); //temp capsule sphere
		Transform tempSphereTransform = Transform();
		tempSphereTransform.SetPosition(d);
		tempSphereTransform.SetOrientation(orientation);
		tempSphereTransform.SetScale(worldTransformA.GetScale());

		return SphereIntersection(tempSphere, tempSphereTransform, volumeB, worldTransformB, collisionInfo);
	}
...
bool CollisionDetection::AABBCapsuleIntersection(
	...
	Vector3 closestPointOnBox = Maths::Clamp(distance, -boxSize, boxSize);
	Vector3 localPoint = distance - closestPointOnBox;
	float length = localPoint.Length();

	if (length < volumeA.GetRadius()) {
		SphereVolume tempSphere = SphereVolume(volumeA.GetRadius()); //temp capsule sphere
		Transform tempSphereTransform = Transform();
		tempSphereTransform.SetPosition(d);
		tempSphereTransform.SetOrientation(orientation);
		...
		return AABBSphereIntersection(volumeB, worldTransformB, tempSphere, tempSphereTransform, collisionInfo);
	}
...
bool CollisionDetection::OBBCapsuleIntersection(
	...
	if (length < volumeA.GetRadius() + volumeB.GetHalfDimensions().Length()) {
		SphereVolume tempSphere = SphereVolume(volumeA.GetRadius()); //temp capsule sphere
		Transform tempSphereTransform = Transform();
		tempSphereTransform.SetPosition(d);
		tempSphereTransform.SetOrientation(orientation);
		...
		return OBBSphereIntersection(volumeB, worldTransformB, tempSphere, tempSphereTransform, collisionInfo);
	}
